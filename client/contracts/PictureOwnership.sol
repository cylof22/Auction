...
contract PictureOwnership is PictureBase, ERC721 {
    ...
    ///  _approve() and transferFrom() are used together for putting pictures on auction, and
    ///  there is no value in spamming the log with Approval events in that case.
    function _approve(uint256 _tokenId, address _approved) internal {
        pictureIndexToApproved[_tokenId] = _approved;
    }

    /// @notice Returns the number of pictures owned by a specific address.
        ...
        whenNotPaused
    {
        ...
        // The contract should never own any pictures (except very briefly
        // after a gen0 cat is created and before it goes on auction).
        require(_to != address(this));
        // Disallow transfers to the auction contracts to prevent accidental
        // misuse. Auction contracts should only take ownership of pictures
    ...
    /// @notice Returns the total number of pictures currently in existence.
    /// @dev Required for ERC-721 compliance.
    ...
    {
        owner = pictureIndexToOwner[_tokenId];
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] ownerTokens) {
        ...
        if (tokenCount == 0) {
            ...
            for (catId = 1; catId <= totalCats; catId++) {
                if (pictureIndexToOwner[catId] == _owner) {
